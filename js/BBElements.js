...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *  */
  ...
  for (let i = 0; i < notes.length; i++) {
    ...
    if (aTop || aBottom) {
      ...
      if (aBottom.firstChild) {
        while (aBottom.firstChild) { aBottom.removeChild(aBottom.firstChild) }
      }
    }
  }
  // ...then create notes
  ...
  */
  let hljs = (typeof exports === 'object') ? require('highlightjs') : window.hljs
  ...
  */
  ...
  if (logo) {
    ...
    let svgLogo
    if (markOnly === 'true') {
      let w = (logo.getAttribute('width')) ? logo.getAttribute('width') : 44
